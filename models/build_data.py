...
def data_generator(dir, entpair2id, path2id):
    """
    ...
    with open(dir+"path_pos_neg.txt") as f:
        ...
        for line in f.readlines():
            ...
            yield path2id[path], entpair2id[pos], entpair2id[neg]



def random_train_dev_split(data, dev_ratio, batch_size, gpu=True):
    """
    ...
    if gpu :
        t_data_tensor = torch.LongTensor(train_data[:,0:3].tolist()).cuda()
        train = data_utils.TensorDataset(t_data_tensor)

        d_data_tensor = torch.LongTensor(dev_data[:,0:3].tolist()).cuda()
        ...
        t_data_tensor = torch.LongTensor(train_data[:, 0:3].tolist())
        train = data_utils.TensorDataset(t_data_tensor)

        d_data_tensor = torch.LongTensor(dev_data[:, 0:3].tolist())
        dev = data_utils.TensorDataset(d_data_tensor)
